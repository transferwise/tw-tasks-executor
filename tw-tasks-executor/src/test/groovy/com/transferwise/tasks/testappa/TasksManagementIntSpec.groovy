...
class TasksManagementIntSpec extends BaseIntSpec {
    ...
    def "erronoeus tasks will be correctly found"() {
        ...
        transactionsHelper.withTransaction().asNew().call {
            TaskTestBuilder.newTask().inStatus(TaskStatus.ERROR).withMaxStuckTime(ZonedDateTime.now().plusDays(1)).build()
        }
        UUID task1Id = transactionsHelper.withTransaction().asNew().call {
            TaskTestBuilder.newTask().inStatus(TaskStatus.ERROR).withType("T1").withSubType("S1")
    ...
    def "stuck tasks will be correctly found"() {
        ...
        UUID task1Id = transactionsHelper.withTransaction().asNew().call {
            TaskTestBuilder.newTask().inStatus(TaskStatus.SUBMITTED).withMaxStuckTime(ZonedDateTime.now().minusDays(1)).build()
        }
        when:
    ...
    def "find a task in #status state"() {
        ...
        UUID taskId = transactionsHelper.withTransaction().asNew().call {
            TaskTestBuilder.newTask().inStatus(status).withMaxStuckTime(ZonedDateTime.now().minusDays(2)).build()
        }
        when:
