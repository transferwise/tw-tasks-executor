...
/**
 * This message to task converter can be used in cases when you are not completely sure that messages in topic
 * have particular structure and want to handle transformation yourself. {@link KafkaMessageToTaskConverter} will
 * try to transform message in the loop with delay, blocking processing of all messages that came after the one that is being
 * processed. Effectively meaning, blocking endless loop which will never resolve.
 */
