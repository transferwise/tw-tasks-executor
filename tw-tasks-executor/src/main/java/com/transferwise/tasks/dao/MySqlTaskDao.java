...
public class MySqlTaskDao implements ITaskDao {
    ...
    @PostConstruct
    @SuppressWarnings({"checkstyle:multiplestringliterals", "checkstyle:indentation"})
    public void init() {
        ...
        insertUniqueTaskKeySql = "insert ignore into " + uniqueTaskKeyTable + "(task_id,key_hash,`key`) values" +
            "(?, ?, ?)";
        setToBeRetriedSql = "update " + taskTable + " set status=?,next_event_time=?,state_time=?,time_updated=?" +
        ...
        setStatusSql = "update " + taskTable + " set status=?,state_time=?,time_updated=?,version=? where id=?" +
            " and version=?";
        setStatusSql1 = "update " + taskTable + " set status=?,next_event_time=?,state_time=?,time_updated=?" +
            ",version=? where id=? and version=?";
        ...
        prepareStuckOnProcessingTaskForResumingSql = "select id,version,type,priority from " + taskTable +
            " where status=? and processing_client_id=?";
        ...
        findTasksByTypeSubTypeAndStatusSqls = new String[]{"select id,type,sub_type,data,status,version" +
            ",processing_tries_count,priority from " + taskTable + " where type=?", " and status in (??)",
            " and sub_type=?"};
        ...
        getTaskSql1 = "select id,version,type,status,priority,sub_type,data" +
            ",processing_tries_count from " + taskTable + " where id=?";
        ...
        deleteTasksSqls = new String[]{"select id,version from " + taskTable + " where type=?", " and sub_type=?",
            " and status in (??)"};
        ...
        deleteFinishedOldTasksSql = "select id,version from " + taskTable + " where status=? and next_event_time<?" +
            " order by next_event_time limit ?";
        ...
        getTasksInProcessingOrWaitingStatusSql = "select id,version,state_time,type,sub_type,status from " + taskTable +
            " where status in('" + TaskStatus.WAITING.name() + "','" + TaskStatus.PROCESSING + "')" +
            " order by next_event_time desc limit ?";
        getStuckTasksSql1 = "select id,version,next_event_time from " + taskTable + " where status in ('"
            ...
            "','" + TaskStatus.PROCESSING.name() + "') and next_event_time<? order by next_event_time desc" +
            " limit ?";
        clearPayloadAndMarkDoneSql = "update " + taskTable + " set data='',status=?,state_time=?,time_updated=?" +
            ",version=? where id=? and version=?";
    ...
    public InsertTaskResponse insertTask(InsertTaskRequest request) {
        ...
        ZonedDateTime nextEventTime = request.getRunAfterTime() == null ?
            request.getMaxStuckTime() : request.getRunAfterTime();
    ...
    public List<Task> findTasksByTypeSubTypeAndStatus(String type, String subType, TaskStatus... statuses) {
        String sql = cachedSql(sqlKey("findTasksByTypeSubTypeAndStatus", subType == null ? 0 : 1,
    ...
    @Override
    /*
      NB. This performs well only with one status. Support for multiple status are meant for test usages.
      We need to use UNION ALL if we want to support production calls as well with multiple statuses.
     */
    ...
    @Override
    //TODO: This does not perform well, at least on MySQL 5.6. "in (status1, status2) order by next_event_time" will not use "(status,next_event_time)" index.
