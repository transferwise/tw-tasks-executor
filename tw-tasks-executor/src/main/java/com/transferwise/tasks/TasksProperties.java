...
public class TasksProperties {
    ...
    /**
     * Generic maximum time to wait for any lock, event or polling. It helps to make the system more robust and better debuggable.
     * Usually you will never want to change this.
     */
    ...
    /**
     * We have two triggering algorithms.
     * First one spreads task triggerings into partitions and every service node is listening only its own partition. The cons is that if one node gets very slow, some tasks latency goes up,
     * even when other nodes would have processing power for them.
    ...
    /**
     * We support Transferwise Kafka failover, where for every topic, we additionally listen to 2 other topics, one starting with "fra." and other with ".aws.".
     * <p>
     * e.g. kafkaDataCenterPrefixes = "fra.,aws.";
     */
    private String kafkaDataCenterPrefixes = "";
    /**
     * Sometimes environments and engineers are forced to use same Kafka server, but still want to deal with only their own messages.
     * In that case we can configure a so called namespace string, which is prepended to every topics name.
     */
    private String kafkaTopicsNamespace;
    /**
     * When listening Kafka Topics, it is possible to specify the topics replication factor and partitions count, which is applied on application startup.
     * <p>
     * It may add a small additional time for startup, which is sometimes prefferred to be avoided (integration tests, development).
     * <p>
     * Using this option, the automatic topic configuration can be turned off.
     */
    private boolean coreKafkaListenerTopicsConfiguringEnabled = true;
    /**
     * Set it to false, if you use proper transaction manager and not spring's default one.
     * You will get better performance and waste less memory.
     * Default option true prevents possible deadlocks with any transaction manager.
     */
    private boolean asyncTaskTriggering = true;
    /**
     * Tied to the previous option. If asyncTaskTriggering is enabled, how many triggerings to we keep in memory, before starting to throttle new tasks added.
     */
    ...
    /**
     * How many old tasks maximum do we delete in one batch/transaction. Deletion should always happen in small batches to not create too big spikes for database replication.
     * <p>
     * This can handle 10 tasks/s.
     * <p>
     * TODO: Implement dynamic, adaptive configuration/system for that instead. Batch Size could be constant, but interval should learn from current situation.
     * Can use TCP/IP flow control algorithms.
     */
     ...
     * <p>
     * If a task handler is configured with a bucket not present in this list, then the handler will not be invoked
     * when new tasks of the configured type are submitted, and instead the task will be sent to the error state.
     */
    ...
    /**
     * For extremely latency sensitive scenarios or for tests, we allow to trigger directly in the same process, instead of going through the kafak pipes.
     * TODO: Maybe allow to execute service tests without having no Kafka at all.
     * Probably best to have a separate ITasksExecutionTrigger implementation instead of hacking it into Kafka one.
     */
