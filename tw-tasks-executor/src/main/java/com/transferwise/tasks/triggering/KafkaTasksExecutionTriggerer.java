...
public class KafkaTasksExecutionTriggerer implements ITasksExecutionTriggerer, GracefulShutdownStrategy {
    ...
    protected ConsumerBucket getConsumerBucket(String bucketId) {
        return ExceptionUtils.doUnchecked(() -> {
            ...
            if (consumerBucket == null) {
                ...
                meterHelper.registerGauge(METRIC_PREFIX + "kafkaTasksExecutionTriggerer.offsetsToBeCommitedCount", tags, finalCb::getOffsetsToBeCommitedCount);
                ...
                meterHelper.registerGauge(METRIC_PREFIX + "kafkaTasksExecutionTriggerer.unprocessedFetchedRecordsCount", tags, finalCb::getUnprocessedFetchedRecordsCount);
    ...
    public void poll(String bucketId) {
        ...
        try {
            ...
            while (!shuttingDown && (getProcessingBucket(bucketId).getState() == ITasksService.TasksProcessingState.STARTED)) {
                ...
                for (ConsumerRecord<String, String> consumerRecord : consumerRecords) {
                    ...
                    MdcContext.with(() -> {
                        ...
                        meterHelper.incrementCounter(METRIC_PREFIX + "kafkaTasksExecutionTriggerer.receivedTriggersCount", ImmutableMap.of("bucketId", bucketId), 1);
                        while (!shuttingDown) {
                            ...
                            /*
                                ...
                                The whole algorithm in this method needs to be overlooked, because of this. Probably some kind of KafkaConsumer.cancel/pause approach needs to be used.
            ...
            // Notice, that now commits will not work anymore. However that is ok, we prefer to unsubscribe, so other nodes can take the partitions to themselves asap.
    ...
    private void releaseCompletedOffset(ConsumerBucket consumerBucket, TopicPartition topicPartition, long offset) {
        ...
        try {
            ...
            if (!offsets.contains(offset)) {
                // Theoretically possible, when we reconnect to Kafka and we had registered one offset multiple times (in this case there is only single record in TreeSet for it).
                log.warn("Offset " + offset + " has already been commited.");
                return;
            }
